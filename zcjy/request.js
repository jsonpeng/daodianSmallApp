...
function request(app_obj, api_url, callback, api_data = {}, token = false, method = 'GET',form = false) {
  if (token || api_data === true ) {
    api_url = api_url + '?token=' + app_obj.globalData.token;
  }
  api_data['shop_id'] = app_obj.globalData.shop_id;
  cache.AutoVarifyCache('now_shop', function (e) {
    if (e) {
      api_data['shop_id'] = e.id
    }
  });
  var header = !form ? {
